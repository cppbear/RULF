...
A patched version of [RuMono](https://github.com/Artisan-Lab/RULF/tree/RuMono) in Artisan-Lab/RULF.
...
4. To build our tool, run `scripts/build-in-docker` at the project directory. This process will build the Rust toolchains as well as RuMono.
...
4. Run `rumono fuzz [-l <LOOPCOUNT>] [-t <TIMEOUT>]` to start fuzzing all fuzz drivers. `-l <LOOPCOUNT>` is needed for collecting coverage information, and `-t <TIMEOUT>` is used to control the time of fuzz in seconds.
